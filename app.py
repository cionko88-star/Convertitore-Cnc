...
def traduci_selca_in_iso(codice_selca: str, nome_prog: str = "200011974-A") -> str:
    ...
    while idx < len(righe):
        ...
        if riga_p.startswith('['):
            # Spegniamo M5/M9 sui commenti solo se non siamo nel bel lavorazioni consecutive dello stesso utensile (es. T6 SMUSSI M6 -> SMUSSI CAVE)
        ...
        if re.search(r'\bS\d+\s+M3\b', clean):
            ...
            if info:
                ...
                # Se siamo su S9000 M3 (SMUSSI CAVE del T6), evitiamo di rimettere T e refrigerante se già attivi
