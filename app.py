...
def traduci_selca_in_iso(codice_selca: str, nome_prog: str = "200011974-A") -> str:
    ...
    primo_utensile_incontrato = True
    ...
    while idx < len(righe):
        ...
        # Cambio Utensile (M6): inserisce M5 ed M9 PRIMA del cambio utensile, tranne che per T1 / primo utensile
        if re.search(r'\bT\d+\s+M6\b', clean) or (re.search(r'\bT\d+\b', clean) and 'M6' in clean):
            m_t = re.search(r'T(\d+)', clean)
            if m_t:
                t_num = int(m_t.group(1))
                if not primo_utensile_incontrato and t_num != 1:
                    righe_iso.append(f"N{n_linea} M5")
                    n_linea += 2
                    righe_iso.append(f"N{n_linea} M9")
                    n_linea += 2
                
                primo_utensile_incontrato = False
                utensile_attuale = t_num
    ...
    # Chiusura finale obbligatoria per l'ultimissimo utensile del programma
    if righe_iso:
        righe_iso.append(f"N{n_linea} M5")
        n_linea += 2
        righe_iso.append(f"N{n_linea} M9")
