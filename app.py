...
<head>
    <meta charset="UTF-8">
    <title>ISO ↔ SELCA Convertitore</title>
    <style>
            ...
            <div class="workspace">
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">➔ Fonte ISO</span>
                    </div>
                    <textarea name="codice_iso" placeholder="Incolla il tuo programma qui...">{{ codice_iso }}</textarea>
                    <div class="actions">
                ...
                <div class="card">
                    <div class="card-header">
                        <span class="card-title">∿ Uscita SELCA</span>
                    </div>
                    <textarea class="output" readonly placeholder="I tuoi blocchi convertiti atterreranno qui.">{{ codice_selca }}</textarea>
        ...
        function caricaFile(input) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    document.querySelector("textarea[name='codice_iso']").value = e.target.result;
                };
...
@app.route('/')
def index():
    return render_template_string(HTML_TEMPLATE, codice_iso="", codice_selca="")

@app.route('/converti', methods=['POST'])
def converti():
    codice_iso = request.form.get('codice_iso', '')
    codice_selca = traduci_iso_in_selca(codice_iso)
    return render_template_string(HTML_TEMPLATE, codice_iso=codice_iso, codice_selca=codice_selca)
...
def scarica():
    ...
    return Response(
        codice_selca,
        mimetype="text/plain",
        headers={"Content-disposition": "attachment; filename=programma_selca.nc"}
    )
